...
class SignUpForm extends Component<{email: string}, {currentStep: number,
    ...
    async onSubmit(values: SignUpFormSchema, helpers: FormikHelpers<SignUpFormSchema>) {
        ...
        // }
            ...
            helpers.setSubmitting(false);
            // Assuming `e` could be of any type, we first assert it to be of type `Error & { response?: AxiosResponse }`
            // This allows us to safely check if `response` exists on `e`
            const error = e as Error & { response?: AxiosResponse };
            if (error.response) {
                const { response } = error; // Now TypeScript knows `response` is of type `AxiosResponse`
                const data = response.data;
                if (response.status === 409) {
                    this.setState({ errorMessage: data });
                } else {
